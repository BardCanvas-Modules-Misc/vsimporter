...
$raw_sources = $settings->get("modules:vsimporter.feeds");
if( empty($raw_sources) )
{
    cli_colortags::write(
        "<red>[$now] Critical: source feeds unset</red>\n" .
        "<red>Please open the settings editor and define the VS feeds to fetch.</red>\n" .
        "<red>Aborting.</red>\n\n"
    );
...
foreach(explode("\n", $raw_sources) as $line)
{
    ...
    $feed_sources[$name] = (object) array(
        "url"      => trim($parts[1]),
        "prefix"   => trim($parts[2]),
        "last_pid" => "",
    );
...
if( ! empty($raw_pids) )
{
    ...
    {
        $line = trim($line);
        $name = trim(current(explode(":", $line)));
        if( isset($feed_sources[$name]) ) $feed_sources[$name]->last_pid = trim(end(explode(":", $line)));
    }
}
...
foreach( $feed_sources as $feed_name => $feed_data )
{
    ...
    if( $offset == "" )
    {
        cli_colortags::write("<light_gray> > Fetching feed </light_gray><light_blue>$feed_name</light_blue><light_gray>...</light_gray> ");
    }
    else
    {
        $feed_url .= "&o=$offset";
        cli_colortags::write("<white> > Fetching feed </white><light_blue>$feed_name</light_blue><white> starting from offset $offset...</white> ");
    }
...
$lastpids_value = "";
foreach($feed_sources as $feed_name => $feed_data)
    $lastpids_value .= "{$feed_name}: {$feed_data->last_pid}\n";
$settings->set("modules:vsimporter.last_article_id", $lastpids_value);
